...
const Videos = ({ videos }) => {
  ...
  return (
    <div className='pt-1 mr-auto d-flex flex-row flex-nowrap' style={{ overflowX: "auto" }}>
      {videos.videos.map((video, index) => {
        return (
          <Card
            key={`video_${index}`}
            className='col-8 col-md-5 img-responsive px-0'
            ...
            {showModal && videoId === video.id && (
              <VideoModal showModal={showModal} handleClose={handleModalClose} trailer={video} />
            )}
          </Card>
        );
      })}
