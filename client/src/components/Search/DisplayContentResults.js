      ...
      {content && content.data.length !== 0 && (
        <ul style={{ listStyle: "none" }}>
          {content.data.map((item, index) => {
            return (
              <li key={`search_${type}_${item.id}`} ref={content.data.length === index + 1 ? lastSearchedResult : null}>
                <Card
                  ...
                  <Link
                    ...
                    <Card.Img
                      ...
                      src={item.poster_path === null ? noImage : `https://image.tmdb.org/t/p/w342${item.poster_path}`}
                    />
                  </Link>
                  <Card.Body className='col-md-10 pt-2 pb-3 pb-lg-0'>
                    <div className='d-flex flex-row'>
                      <Link
                        to={`/${type}/${item.id}`}
                        style={{ textDecoration: "none" }}
                        className={item.vote_average !== 0 ? "col-9 px-0" : "col-12 px-0"}>
                        <Card.Title className='mb-2'>{item.title ? item.title : item.name}</Card.Title>
                      </Link>
                      {item.vote_average !== 0 && (
                        <span className='pl-3 col-3 text-center'>
                          {item.vote_average}
                          <FontAwesomeIcon icon={faStar} className='pl-1' color='gold' />
                        </span>
                      )}
                    </div>
