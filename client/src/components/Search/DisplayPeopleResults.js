  ...
  const lastSearchedResult = useCallback(
    (result) => {
      ...
      observer.current = new IntersectionObserver((entries) => {
        if (entries[0].isIntersecting && people.currentPage < people.totalPages) {
    ...
    <div>
      <ul style={{ listStyle: "none" }}>
        {people.data.map((person, index) => {
          return (
            <li key={`search_show_${person.id}`} ref={people.data.length === index + 1 ? lastSearchedResult : null}>
              <Card
                className='d-flex flex-row mb-3'
                style={{ maxHeight: "300px", backgroundColor: "#16161d", color: "white", border: "#30363d 2px solid" }}>
                <Link to={`/person/${person.id}`} className='col-md-3 col-lg-2 px-0' style={{ textDecoration: "none" }}>
                  <Card.Img
                    className='img-responsive'
                    src={
                      person.profile_path === null ? noImage : `https://image.tmdb.org/t/p/w342${person.profile_path}`
                    }
                  />
                </Link>
                <Card.Body className='col-md-10 pt-1'>
                  <div className='d-flex flex-row'>
                    <Link to={`/tv/${person.id}`} style={{ textDecoration: "none" }}>
                      <Card.Title className='mb-2'>{person.name}</Card.Title>
                    </Link>
                  </div>
                  <Card.Subtitle className='text-muted'>{person.known_for_department}</Card.Subtitle>
                  ...
                  {person.known_for[0] && (
                    <Card.Subtitle
                      ...
                      Known for:
                      <span key={`search_people_known_for_${person.known_for[0].id}`} className='pl-1'>
                        <Link
                          to={
