...
const Ratings = ({ ratings, stats, updateRating, removeRating }) => {
  ...
  useEffect(() => {
    ...
    const modified_rating = ratings[selectedCategory].map((item) => {
      return {
        id: item.id,
        title: item.title,
        tmdb_rating: stats[selectedCategory].filter((content) => item.id === content.contentId)[0].tmdb_rating,
    ...
    console.log(content);
    updateData(content);
  }, [ratings, selectedCategory, stats]);

  const imageFormatter = (row, cell) => {
    console.log(cell);
    return (
      <Link to={`/${selectedCategory}/${cell.id}`} style={{ textDecoration: "none" }}>
        <Image
          src={cell === null ? noImage : `https://image.tmdb.org/t/p/w780${cell.backdrop}`}
  ...
  const editRatingFormatter = (row, cell) => {
    ...
    return (
      <span style={{ cursor: "pointer" }}>
        <Rating item={cell} itemType={selectedCategory} handleRating={updateContentRating} type='edit' />
      </span>
    );
  };
  ...
  const deleteRatingFormatter = (row, cell) => {
    ...
    return (
      <span style={{ cursor: "pointer" }}>
        <FontAwesomeIcon icon={faTrash} color='red' onClick={() => deleteContentRating(cell)} />
      </span>
    );
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      formatter: (row, cell) => {
        return (
          <Link to={`/${selectedCategory}/${cell.id}`} style={{ textDecoration: "none", color: "#c3d1d9" }}>
            {cell.title}
          </Link>
        );
      },
            ...
            <Container>
              <Row>
                <Col style={{ color: "#c3d1d9" }}>
                  No {selectedCategory === "movie" ? "Movies" : "TV Shows"} rated yet
                </Col>
              </Row>
            </Container>
          )}
...
export default connect(mapStateToProps, { updateRating, removeRating })(Ratings);
