...
import { getStatsContent, getCustomListsContent } from "../../actions/dashboard";
import Stats from "./Stats/Stats";

const Dashboard = ({ profile, getStatsContent, getCustomListsContent, loading }) => {
  const [section, updateSection] = useState("watched");

  useEffect(() => {
    getStatsContent();
    getCustomListsContent();
  }, [getCustomListsContent, getStatsContent]);

  return (
    <Fragment>
      {profile.loading && <Spinner />}
      {!profile.loading && (
        <Container fluid className='mx-0 w-100 px-0' style={{ backgroundColor: "#090c12" }}>
          <Row className='mx-0 px-0'>
            <Col md={3} className='px-0' style={{ borderRight: "#30363d 2px solid" }}>
